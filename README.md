...
```
packages/
  prodoc-core/      # 核心库 - 文档解析、数据模型、流程图 DSL
  prodoc-renderer/  # 渲染器 - React 组件库（新拟态 UI）
  prodoc-editor/    # 编辑器 - React 编辑器组件库
...
在 Markdown 中使用 `prodoc-flow` 代码块定义流程图：

````markdown
```prodoc-flow
graph LR
  A[开始] --> B{判断}
  B -->|是| C[处理1|/guide/step1.md]
  B -->|否| D[处理2|/guide/step2.md]
  C --> E[结束]
  D --> E
```
````
...
| 组件 | 说明 | 特性 |
|------|------|------|
| `DocViewer` | 文档查看器（侧边栏 + 内容区） | 主题切换、搜索过滤 |
| `TreeNavigator` | 树状文档导航 | 展开/折叠、搜索高亮 |
| `MarkdownRenderer` | Markdown 渲染器 | 目录导航、代码高亮、流程图 |
| `FlowRenderer` | 流程图渲染器 | 拖拽平移、滚轮缩放、节点跳转 |
...
主题状态通过 `data-theme` 属性和 `pd-theme-dark` 类名应用到 `document.documentElement`，所有组件自动响应主题变化。
