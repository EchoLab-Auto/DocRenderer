...
const NPM_ALIAS = 'npm:@echolab-auto/ui-frame@^1.0.0';
const FILE_PROTOCOL = 'file:vendor/@echolab/ui-frame';
const FILE_PROTOCOL_SUB = 'file:../../vendor/@echolab/ui-frame';
...
function switchTo(mode) {
  ...
  for (const { file, field } of PKG_FILES) {
    ...
    if (!pkg[field] || !pkg[field]['@echolab/ui-frame']) {
      console.warn(`⚠️  @echolab/ui-frame not found in ${file} > ${field}, skipping.`);
      continue;
    }

    if (isFileMode) {
      // 根据层级选择正确的相对路径
      pkg[field]['@echolab/ui-frame'] = file === 'package.json' ? FILE_PROTOCOL : FILE_PROTOCOL_SUB;
    } else {
      pkg[field]['@echolab/ui-frame'] = NPM_ALIAS;
    }
  ...
  if (isFileMode) {
    console.log('\n📦 Building @echolab/ui-frame from source...');
    try {
    ...
    console.log('\n📥 Run "npm install" to install @echolab/ui-frame from npm registry.');
  }
}
